...
class Statistics extends CI_Model {
	...
	private function build_statistics($teams) {
		foreach($this->teams as $team) {
			$this->team_statistics[$team->get_id()] = new Team_statistics($this->get_league(), $team);
		}
		foreach($this->players as $player) {
			$this->player_statistics[$player->get_id()] = new Player_statistics($this->get_league(), $player);
		}
	}
}
?>