<?php
class Team_match extends CI_Model {
	...
	public function get_rounds() {
		if (!isset($this->rounds)) {
			...
			foreach ($query->result() as $round) {
				array_push($rounds, new Team_round($round->round_id, $round->match_id, $round->player_one, $round->player_two, $round->winner));
			}
			$this->rounds = $rounds;
		}
		return $this->rounds;
	...
	public function get_match($id) {
		...
		$match = $query->row();
		return new Team_match($match->match_id, $match->team_one, $match->team_two, $match->date_start, $match->league_id, $match->lineup_position, $match->winner);
	}
